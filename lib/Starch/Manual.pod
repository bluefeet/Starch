...
Starch::Manual - Organized collection of Starch documentation.

=head1 INTRODUCTION

Welcome to Starch, a fancy implementation agnostic session state
manager.  Sessions are server-side buckets of arbitrary information
which is tracked using some implementation-specific technology, such
as HTTP cookies.  While this modules uses are extend far beyind simply
making HTTP requests stateful, this documentation uses this paradigm
extensively to explain how Starch works.

Documentation for extending Starch can be found at
L<Starch::Manual::Extending>.
...
the underlying L<Starch> object and provide a thin layer over
the L<Starch::Session> objects.  This documentation will be using
...
C<Starch> is an attempt at creating a standalone session state
...
store you will not be interacting with it as the L<Starch::Session>
...
This creates a new L<Starch::Session> object which you can then
interact with:

    $session->data->{some_key} = 'some_value';

The L<Starch::Session/data> attribute is a writeable hash ref
...
Expiration can be specified globally, when instantiating the L<Starch>
object, per-session, and per-store.  The expires value has
various properties and behaviors that are important to understand:
...
The L<Starch> class accepts an C<expires> argument which is used
...
the global expires set in the L<Starch> object.  Each session
can then have their individual expire extended or reduced via the
L<Starch::Session/set_expires> method.
...
will be logged which will look something like:
    ...
    Starch session cf1a6abd623f019df53d9a69033b4472366e4afe was changed and not saved.
...
The Starch manager (L<Starch>) and stores support method proxies
...
Plugins alter the behavior of the manager (L<Starch>),
session (L<Starch:Session>), and store (L<Starch::Store>)
